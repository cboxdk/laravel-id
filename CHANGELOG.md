...
- Platform checkpoints are handed to the package's `CheckpointAnchor`, so they can be exported to an external disk (`AUDIT_CHAIN_ANCHOR=filesystem`). The default still exports nothing.
- `DatabaseAuditLog::__construct()` accepts an optional `CheckpointAnchor`; `ChainCheckpoint::fromOutcome()`.
