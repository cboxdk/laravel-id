...
| `Api` | 🔨 | ✅ Machine endpoints (HTTP-tested): `/.well-known/jwks.json`, OIDC discovery, **`POST /oauth/token`** (authorization_code+PKCE → access_token + signed id_token; client_credentials), `POST /oauth/introspect`, `/up`. 11 tests. **SAML ACS ✅** (`POST /sso/saml/{connection}/acs`, unauthenticated — the XML signature is the auth): resolves the connection, validates via `AssertionValidator`, drives `FederationFlow` → session; onelogin's Destination/Recipient check is pinned to the connection's configured `sp_acs_url` (proxy-safe, host-independent). 5 HTTP tests incl. genuinely-signed happy path, untrusted-key/unknown/inactive/missing. **SCIM HTTP endpoint ✅ (full Okta/Entra lifecycle):** bearer-authed `/scim/v2/Users` — filtered **list** (`filter=userName/externalId eq …`, `startIndex`/`count` pagination, ListResponse; unsupported filters → `400 invalidFilter`), create, read, **PATCH** (active + core attributes via path & pathless value-object ops), **PUT** full replace, delete; deprovision revokes sessions end-to-end. 12 tests. **Follow-up:** REST CRUD + OpenAPI, interactive OIDC authorize (consent, SaaS), SAML ACS routes. |
