...
| `Federation` | 🔨 | ✅ Connections (per-org IdP config sealed via Crypto, per-connection routing, active-only resolution) + login orchestration (validated principal → user + membership + session; dogfoods Identity/Organization). `AssertionValidator` contract defines the crypto boundary. 5 tests. **Follow-up:** SAML/OIDC `AssertionValidator` impls wrapping vetted libs (onelogin/php-saml) + conformance. |
...
| `OAuthServer` | 🔨 | ✅ Clients + service accounts (secrets hashed), **client-credentials/M2M** token issuance + introspection + revocation — stateless RS256 JWTs signed & alg-allowlist-verified by the Crypto kernel, `jti`-tracked for revocation. Events + audit. Ships `InteractsWithOAuth`. 8 tests. **Follow-up:** interactive authorization-code/PKCE/OIDC-provider flow (discovery/id_token/userinfo) on league/oauth2-server in the `Api` layer. |
...
| `Api` | 🔨 | ✅ Machine endpoints (HTTP-tested): `/.well-known/jwks.json`, OIDC discovery, `POST /oauth/introspect`, `/up`. 5 tests. **SCIM HTTP endpoint ✅** (bearer-authed `/scim/v2/Users`: create/read/deactivate-via-PATCH/delete; deprovision revokes sessions end-to-end). **Follow-up:** REST CRUD + OpenAPI, interactive OIDC authorize/token (league), fuller SCIM PATCH/filter, SAML ACS routes. |
