...
| `TokenVault` | ✅ | AI token vault: downstream third-party credentials **sealed** via Crypto `SecretBox` (recoverable, AEAD row-bound — not hashed, the vault must replay them) + **deny-by-default leased access** to agent clients (`SecretVault`: store/rotate/revoke/grant/revokeGrant/lease). Uniform `LeaseDenied` (no enumeration oracle; reason audited only), advisory lease TTL capped per-grant, every op audited without the value, environment-owned. Ships `InteractsWithTokenVault` + `FakeTokenVault`. 18 tests (lifecycle, access deny-by-default, environment isolation, sealed-at-rest + AEAD binding). **Honest scope:** lease TTL advisory; master-key rotation is a manual re-seal (`key_version` column). |
