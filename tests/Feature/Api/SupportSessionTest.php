...
function refusalOf(Closure $attempt): ?SupportSessionRefusal
{
    try {
        $attempt();
...
it('refuses a session without a reason', function (): void {
    ...
    expect(refusalOf(fn () => app(SupportSessions::class)->begin(supportRequest($client, $organization, ['reason' => '   ']))))
...
it('refuses staff who do not hold support:impersonate environment-wide', function (): void {
    ...
    expect(refusalOf(fn () => app(SupportSessions::class)->begin(supportRequest($client, $organization, ['actorId' => 'staff-2']))))
...
it('refuses support:impersonate declared by another app or by nobody', function (): void {
    ...
    foreach (['staff-2', 'staff-3'] as $actor) {
        expect(refusalOf(fn () => app(SupportSessions::class)->begin(supportRequest($client, $organization, ['actorId' => $actor]))))
            ->toBe(SupportSessionRefusal::NotPermitted);
    }
})->group('security');
...
it('refuses acting as yourself', function (): void {
    ...
    expect(refusalOf(fn () => app(SupportSessions::class)->begin(supportRequest($client, $organization, ['targetUserId' => 'staff-1']))))
...
it('refuses a target who is not an active member of the organization', function (?MembershipStatus $status): void {
    ...
    expect(refusalOf(fn () => app(SupportSessions::class)->begin(supportRequest($client, $organization, ['targetUserId' => 'customer-2']))))
...
it('refuses a suspended organization', function (): void {
    ...
    expect(refusalOf(fn () => app(SupportSessions::class)->begin(supportRequest($client, $organization))))
...
it('refuses an app that is not a first-party app of the environment', function (bool $firstParty, bool $customerOwned): void {
    ...
    expect(refusalOf(fn () => app(SupportSessions::class)->begin(supportRequest($app, $organization, ['actorId' => 'staff-9']))))
...
it('refuses an unregistered redirect URI and records nothing', function (): void {
    ...
    expect(refusalOf(fn () => app(SupportSessions::class)->begin(
...
it('kills the session’s codes and tokens when it ends', function (): void {
    ...
    expect(refusalOf(fn () => $sessions->issueCode($started->session->id, 'staff-1', supportCode())))
...
it('mints further codes only for the session’s own actor', function (): void {
    ...
    expect(refusalOf(fn () => app(SupportSessions::class)->issueCode($started->session->id, 'staff-2', supportCode())))
        ->toBe(SupportSessionRefusal::SessionNotActive);
