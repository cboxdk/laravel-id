...
- [x] **Phase 5a — OAuth surface data.** oauth_clients, connections, directories, oauth_refresh_tokens, oauth_authorization_codes, device_codes gain environment_id and their models are environment-owned — a client/connection/directory/opaque-token from one environment is invisible (and unusable) from another. Proven by OAuthEnvironmentTest.
- [x] **Phase 5b — Environment resolution.** EnvironmentResolver contract + DatabaseEnvironmentResolver (custom-domain, else leading-subdomain-as-slug); ResolveEnvironment middleware pins the environment for every API request (config default for single-tenant; unknown host refused). environments gains a domain column. Proven by EnvironmentResolutionTest. (Per-environment issuer/discovery string is the remaining sub-item.)
