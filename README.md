...
| **OpenID Connect** | **Full** discovery, JWKS, UserInfo, RP-Initiated Logout, Back-Channel Logout · **Partial** id_token (code flow + `query` mode only), CIBA (poll only) · **Host** `/authorize`, consent, `prompt`/`max_age`/`acr_values` gating, RFC 9207 `iss` · **No** front-channel logout, pairwise `sub` |
