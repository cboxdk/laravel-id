...
| `ExternalActions` | `ActionPipeline`, `ExternalActions` | Inline hooks: synchronous extension points that enrich or veto an operation — in-process handlers or signed, SSRF-guarded external HTTP calls, fail-closed. Flagship: the `TokenMinting` hook (enrich claims / veto issuance). |
