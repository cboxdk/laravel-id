...
| **RFC 8693** — Token Exchange | `urn:ietf:params:oauth:grant-type:token-exchange` | **Partial** | Access-token→access-token only, down-scope-only, with `resource` re-audiencing and DPoP continuity. **No delegation through the exchange grant**: `actor_token` and `may_act` are absent, as are the `id_token` and `saml2` token types, and a subject token carrying `act` is refused. The **`act` claim (§4.1)** is emitted on access tokens, ID Tokens and introspection for [support sessions](support-sessions.md) only — never by exchange. |
...
| **RFC 8707** — Resource Indicators | `resource` binds the access token's `aud` | **Partial** | **Single-valued only** — §2 permits repeating `resource`, and a second value is dropped. Not advertised in metadata (there is no registered metadata key), so this is under-advertised rather than over-advertised. A `resource` naming a [registered API](../core-concepts/apis-and-scopes.md) narrows the token to that API's scopes; scopes of several APIs with no `resource` is `invalid_target`, and nothing grantable for the audience is `invalid_scope`. |
| **RFC 7591** — Dynamic Client Registration | `POST /oauth/register` | **Partial** | Three modes (`disabled` / `protected` / `open`), advertised only when enabled. Redirect-URI rules include RFC 8252 private-use schemes and loopback. Requested scopes are reduced to the configured allow-list plus the tenant-requestable scopes of environment-owned [registered APIs](../core-concepts/apis-and-scopes.md), and the response's `scope` states what was kept. Every method in `token_endpoint_auth_methods_supported` is registrable, `private_key_jwt` included, with the key set supplied inline as `jwks`. **`jwks_uri` is refused** rather than ignored: fetching a registrant-chosen URL from an endpoint that is unauthenticated in `open` mode is SSRF handed out by a public API. |
...
| **OIDC Back-Channel Logout 1.0** | Logout token, `sid` claim, `backchannel_logout_uri` / `backchannel_logout_session_required` | **Full** · host passes the session id | Logout tokens per §2.4 (`iss`, `aud`, `iat`, `exp` two minutes out, per-attempt `jti`, the `events` member, `sub` and/or `sid`, never `nonce`, `typ: logout+jwt`), signed with the environment's ID Token key. Delivered by a queued job over the SSRF guard with pinned DNS and no redirects, retried with backoff, every outcome audited. `sid` is a SHA-256 derivation of the session id, not the id itself. Triggered by every session revocation, by grant revocation, and by removal from an organization. **The host must pass its session id to `AuthorizationCodes::issue()`** for `sid` to exist; without it, relying parties are reachable by `sub` only. See [Receive back-channel logout](../cookbook/receive-back-channel-logout.md). |
